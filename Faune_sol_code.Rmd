...
plotgg1 <- ggplot() + 
  ...
  geom_mark_ellipse(data=sites.long1, 
                    aes(x=axis1, y=axis2, colour=sites.long1$Traitement, 
                        fill=after_scale(alpha(colour, 0.2))), 
                    expand=0, size=0.2, show.legend=FALSE)+ 
  geom_point(data=sites.long1, 
             aes(x=axis1, y=axis2, colour=sites.long1$Traitement, shape=sites.long1$Traitement), 
             size=5) +
...
mat_bota_small <- aggregate(.~Numero_quadrat, data_small, sum)
...
biplot(bp.sym, which = "y1", main = "Faune du sol")
biplot(bp.sym, which = "y2", main = "Bota")
layout(1)
```
