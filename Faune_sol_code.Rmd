...
```{r}
alpha <- diversity(matrice, index = 'shannon', mat$Traitement)
alpha_1 <- diversity(matrice, index = 'shannon', mat$Parcelle)

mat$alpha <- ifelse(mat$Traitement=='Ligneux', 1.39664, 1.645358)
mat$alpha_1 <- ifelse(mat$Parcelle=='B_L',1.282906,
                      ifelse(mat$Parcelle=='B_NL',1.214442,
                      ifelse(mat$Parcelle=='M_L', 1.509221,
                      ifelse(mat$Parcelle=='M_NL',1.848948,
                      ifelse(mat$Parcelle=='T_L',1.220084, 1.517253)))))
```                  
...
ggplot(mat,aes(y=alpha_1,x=Traitement,fill=Traitement))+
  geom_boxplot()+
  stat_summary(fun = mean, geom = "point", col = "red") +
  theme_bw()
```
When I comput mean on each field (6 fields), the main obtained is not the same that when I compute mean on the treatments. The issue is that I want the mean difference between each treatments, but when I compute it directly, I cant have sd, and so I cant plot it and test it via t.test() for examples. So I dont know what to do.
